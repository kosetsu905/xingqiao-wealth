...
@RocketMQMessageListener(
        topic = "order_topic",
        selectorExpression = "order_completed",
        consumerGroup = "order_completed_group")
public class OrderCompletedConsumer implements RocketMQListener<MessageExt> {
     ...
     * 处理订单完成消息的业务逻辑
     */
    private void processOrderCompletedMessage(OrderMessage orderMessage) {
        logger.info("开始处理订单完成消息，订单ID: {}", orderMessage.getOrderId());
        
        try {
            // 这里可以添加订单完成后的业务逻辑
            // 例如：更新订单状态、通知相关系统、发送短信/邮件通知等
            
            // 模拟业务处理
            logger.info("订单完成处理成功，订单ID: {}, 客户ID: {}, 订单金额: {}",
                    orderMessage.getOrderId(),
                    orderMessage.getCustomerId(),
                    orderMessage.getAmount());
            
        } catch (Exception e) {
            logger.error("订单完成处理异常，订单ID: {}", orderMessage.getOrderId(), e);
            // 可以根据具体业务需求决定是否抛出异常，触发重试机制
            // 如果需要重试，可以抛出异常；否则记录日志并返回
            // throw new RuntimeException("订单完成处理失败", e);
        }
    }
}