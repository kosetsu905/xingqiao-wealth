...
public class AlphaVantageService {
     ...
     * 获取全球市场概览
     */
    public List<Map<String, Object>> getGlobalMarkets() {
        // 市场列表和对应的 AlphaVantage Symbol
        Map<String, String> markets = new HashMap<>();
        markets.put("BITCOIN", "BTC");
        markets.put("ETHER", "ETH");
        markets.put("SOLANA", "SOL");
        markets.put("Apple", "AAPL");
        markets.put("Microsoft", "MSFT");
        markets.put("Google", "GOOG");
        markets.put("Amazon", "AMZN");
        markets.put("Tesla", "TSLA");
        ...
        for (Map.Entry<String, String> entry : markets.entrySet()) {
            ...
            try {
                ...
                String percentStr = quote.has("10. change percent") ? quote.get("10. change percent").asText() : "";

                Map<String, Object> market = new HashMap<>();
                market.put("name", name);
                market.put("code", symbol);
                market.put("value", String.format("%,.2f", price));
                market.put("change", (change >= 0 ? "+" : "") + String.format("%,.2f", change));
                market.put("percent", percentStr);
                market.put("changeClass", change >= 0 ? "text-green-600" : "text-red-600");
                market.put("bgClass", change >= 0 ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800");

                result.add(market);

            } catch (Exception e) {
                System.err.println("获取全球市场 " + symbol + " 失败: " + e.getMessage());
            }
        }
